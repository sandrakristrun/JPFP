...
export class AllCampuses extends React.Component {
  ...
  render() {
    ...
    <div className='campusesList'>
      ...
      <select name='campusFilter' value={filtered} onChange={this.handleChange}>
      <option>All</option>
      ...
      {campuses.map(campus => (
        <div className='campus' key={campus.id}>
          <Link to={`/campuses/${campus.id}`}>
            ...
            <div>
              <button type='submit' id='remove-button'onClick={this.handleClick}value={campus.id}>Remove</button>
            </div>
        </div>
      ))}
      </div>
      <div className='right'>
        <div id='addCampus'><h3>Add New Campus</h3></div>
        <AddCampus/>
      </div>
    </div>
    )
